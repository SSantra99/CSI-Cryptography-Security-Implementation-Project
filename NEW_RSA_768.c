...
#define PRIME_BIT_1 768
#define PRIME_BIT_2 1024
...
int main(void) {
    ...
    for (int t = 0; t < TRIALS; t++) {
        ...
        do {
            ...
            mpz_urandomb(tmp, state, PRIME_BIT_1); //Generating prime of 768 bit
            mpz_setbit(tmp, PRIME_BIT_1 - 1); // guarantee bit-length
        ...
        do {
            start = rdtsc_serialized_begin();
            mpz_urandomb(tmp, state, PRIME_BIT_2);  //Generating prime of 1024 bit
            mpz_setbit(tmp, PRIME_BIT_2 - 1); // guarantee bit-length
            mpz_setbit(tmp, 0); // Setting the LSB to 1
            mpz_nextprime(q, tmp);
            end = rdtsc_serialized_end();
    ...
    printf("Over %d many trials (PRIME_BIT_1=%d,PRIME_BIT_2=%d):\n\n", TRIALS, PRIME_BIT_1,PRIME_BIT_2);
    ...
    do {
        mpz_urandomb(tmp, state, PRIME_BIT_1);
        mpz_setbit(tmp, PRIME_BIT_1 - 1);
        mpz_setbit(tmp, 0);
        mpz_nextprime(p, tmp);
        mpz_sub_ui(tmp, p, 1);
    } while (mpz_divisible_ui_p(tmp, 65537));
    ...
    do {
        mpz_urandomb(tmp, state, PRIME_BIT_2);
        mpz_setbit(tmp, PRIME_BIT_2 - 1);
        mpz_setbit(tmp, 0);
        mpz_nextprime(q, tmp);
        mpz_sub_ui(tmp, q, 1);
    } while (mpz_cmp(p, q) == 0 || mpz_divisible_ui_p(tmp, 65537));
